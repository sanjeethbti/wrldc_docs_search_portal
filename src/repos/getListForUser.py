...
def getListForUser(appDbConnStr: str,userId:int):
    ...
    targetColumns = ['ID', 'REGULATION_NAME', 'REG_TYPE', 'AMENDMENT_NO', 'NOTIFICATION_DATE',
                     ...
                     'PDF_FILE_NAME', 'CERC_SITE_PDF_LINK','KEYWORDS_USER','KID']

    codesFetchSql = """
            select {0}
            from WRLDC_REST_SUBSET.FILE_INFO info 
            LEFT JOIN (
                select * from WRLDC_REST_SUBSET.USER_KEYWORD  where USER_ID = :1 ) 
                key ON info.ID=key.DOC_ID""".format(','.join(targetColumns))
    ...
    try:
        ...
        dbCur = dbConn.cursor()
        dbCur.execute(codesFetchSql,(userId,))
    ...
    for row in dbRows:
        ...
        cerc_Site_pdf_link = row[colNames.index(
            'CERC_SITE_PDF_LINK')]
        keywords_user=row[colNames.index(
            'KEYWORDS_USER')]
        kid=row[colNames.index(
            'KID')]
        docDetail = {
            "id": id,
